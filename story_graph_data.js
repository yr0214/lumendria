window.storyGraphData = {
  "nodes": [
    {
      "id": "start",
      "label": "L0 #1 start\\n당신의 동생은 3년 전 이 왕국에서 죽었다.",
      "title": "당신의 동생은 3년 전 이 왕국에서 죽었다.\n사인은 '저주병' — 왕국 밖에서는 존재하지도 않는 병명이었다.\n당신은 3년을 기다렸다. 조사했다. 그리고 국경을 넘었다.\n복수인지, 진실인지, 당신 스스로도 아직 모른다.\n아에르돈의 성벽이 보인다. 바람에 재 냄새가 섞여 온다.",
      "shape": "box",
      "level": 0,
      "order": 1,
      "ending": false
    },
    {
      "id": "forest",
      "label": "L1 #2 forest\\n아에르돈의 숲, 이름은 '회색의 관'.",
      "title": "아에르돈의 숲, 이름은 '회색의 관'.\n한때 요정들이 노래하던 곳이라 했지만, 지금은 새 한 마리 울지 않는다.\n발소리조차 흙에 흡수되는 느낌.\n그리고 — 나무 한 그루에 낡은 천이 묶여 있다. 누군가 표식을 남겼다.",
      "shape": "box",
      "level": 1,
      "order": 2,
      "ending": false
    },
    {
      "id": "village",
      "label": "L1 #3 village\\n마을 광장. 사람들은 눈을 마주치지 않는다.",
      "title": "마을 광장. 사람들은 눈을 마주치지 않는다.\n벽에는 긁힌 글자들: '왕을 믿지 마라' — 그 옆에 누가 덧붙였다. '마법사도.'\n한 아이가 광장 한가운데 서서 당신을 빤히 본다.\n아이의 눈이 — 동생을 닮았다.",
      "shape": "box",
      "level": 1,
      "order": 3,
      "ending": false
    },
    {
      "id": "castle_gate_early",
      "label": "L1 #4 castle_gate_early\\n성문은 닫혀 있다. 수문장이 창을 겨눈다.",
      "title": "성문은 닫혀 있다. 수문장이 창을 겨눈다.\n'민간인 출입 금지. 왕의 명령이다.'\n당신 뒤로 마을 방향에서 연기가 피어오르는 게 보인다.\n무언가 타고 있다.",
      "shape": "box",
      "level": 1,
      "order": 4,
      "ending": false
    },
    {
      "id": "cave",
      "label": "L2 #5 cave\\n동굴 안. 제단석 위에 검이 꽂혀 있다.",
      "title": "동굴 안. 제단석 위에 검이 꽂혀 있다.\n칼날에 문자: '자격 있는 자만이 이 검의 무게를 안다.'\n그런데 — 제단 옆 바닥에 최근에 누군가 다녀간 흔적이 있다.\n발자국. 작다. 어른이 아니다.",
      "shape": "box",
      "level": 2,
      "order": 5,
      "ending": false
    },
    {
      "id": "deep_forest",
      "label": "L2 #6 deep_forest\\n숲 심장부. 나무들이 점점 검어진다.",
      "title": "숲 심장부. 나무들이 점점 검어진다.\n그리고 — 거대한 회색 늑대가 앞을 막아선다. 눈이 붉다.\n그런데 이상하다. 늑대의 옆구리에 오래된 화살이 박혀 있다.\n달아나거나, 싸우거나, 그 화살을 먼저 볼 것이냐.",
      "shape": "box",
      "level": 2,
      "order": 6,
      "ending": false
    },
    {
      "id": "wizard",
      "label": "L2 #7 wizard\\n마법사의 탑. 그는 등을 보인 채 창밖을 내다보고 있다.",
      "title": "마법사의 탑. 그는 등을 보인 채 창밖을 내다보고 있다.\n'올 줄 알았다.' 먼저 말한다.\n'왕이 저주의 근원이다. 내가 진실을 밝히려다 갇혔지.'\n목소리는 침착하다.\n그러나 당신은 — 이미 너무 많이 알고 있다.",
      "shape": "box",
      "level": 2,
      "order": 7,
      "ending": false
    },
    {
      "id": "villager",
      "label": "L2 #8 villager\\n한 여인이 다가온다. 눈빛이 지쳐 있다.",
      "title": "한 여인이 다가온다. 눈빛이 지쳐 있다.\n'제발 도와주세요. 아이가 사흘째 잠에서 깨지 않아요.\n의원이 말했어요 — 기억을 잃어가고 있다고. 이름도, 얼굴도.'\n당신은 그 말을 듣는 순간 멈춘다.\n기억을 가진 자를 바친다. 아이도 그 대상이다.",
      "shape": "box",
      "level": 2,
      "order": 8,
      "ending": false
    },
    {
      "id": "village_wall",
      "label": "L2 #9 village_wall\\n골목 끝. 노인 하나가 웅크리고 있다.",
      "title": "골목 끝. 노인 하나가 웅크리고 있다.\n당신을 보자 속삭인다:\n'3년 전 외지인이 왔었소. 저주병이라 불렀지만 — 그건 병이 아니었소.\n기억을 가진 자를 고르는 거요. 누군가의 부탁으로. 탑의 마법사.'\n노인의 눈이 멀다. 그는 당신을 보지 못한다. 그러나 알고 있다.",
      "shape": "box",
      "level": 2,
      "order": 9,
      "ending": false
    },
    {
      "id": "gate_talk",
      "label": "L2 #10 gate_talk\\n수문장이 비웃는다.",
      "title": "수문장이 비웃는다.\n'저주를 끊겠다는 자가 한둘이 아니었소. 다들 돌아오지 않았지.'\n그는 잠깐 목소리를 낮춘다.\n'...마법사 탑 쪽에 쪽문이 있소. 내가 못 본 걸로 하겠소.\n대신 — 탑 안에 있는 걸 믿지 마시오.'",
      "shape": "box",
      "level": 2,
      "order": 10,
      "ending": false
    },
    {
      "id": "cave_sword_taken",
      "label": "L3 #11 cave_sword_taken\\n검이 손에 잡히는 순간, 묵직한 울림이 느껴진다.",
      "title": "검이 손에 잡히는 순간, 묵직한 울림이 느껴진다.\n그리고 제단 뒤 벽에 균열이 생기며 문자가 드러난다:\n'이 검은 저주를 끊지 못한다. 그러나 진실을 지키는 자의 손에서는 다르다.'\n당신은 그 말의 의미를 아직 모른다.",
      "shape": "box",
      "level": 3,
      "order": 11,
      "ending": false
    },
    {
      "id": "secret_path",
      "label": "L3 #12 secret_path\\n발자국이 이어지는 곳 — 제단 뒤 숨겨진 통로.",
      "title": "발자국이 이어지는 곳 — 제단 뒤 숨겨진 통로.\n안으로 들어가자 쓰러진 기사의 유해가 있다.\n갑옷에 왕실 친위대 문장. 손에 쥔 양피지:\n'마법사가 왕에게 계약서를 읽어줬다. 그러나 번역은 마법사가 했다.\n왕은 자신이 무엇에 서명했는지 몰랐다.'\n그 아래 — 필체가 흔들린 채로: '이 사실을 알게 된 우리는 하나씩 사라졌다.'",
      "shape": "box",
      "level": 3,
      "order": 12,
      "ending": false
    },
    {
      "id": "cave_inspect",
      "label": "L3 #13 cave_inspect\\n제단 뒤쪽, 바닥에 눌린 흔적들.",
      "title": "제단 뒤쪽, 바닥에 눌린 흔적들.\n그리고 돌 틈에 끼인 단추 하나 — 왕실 문장이 새겨진 금 단추.\n3년 전 왕실 방문단이 착용했던 것과 같은 양식이다.\n당신의 손이 떨린다.",
      "shape": "box",
      "level": 3,
      "order": 13,
      "ending": false
    },
    {
      "id": "wolf_win",
      "label": "L3 #14 wolf_win\\n늑대가 쓰러진다.",
      "title": "늑대가 쓰러진다.\n죽는 순간 털이 하얗게 변하며 붉은 빛이 사라진다.\n그리고 — 당신은 본다. 늑대의 목에 낡은 목줄. 왕실 문장.\n이 늑대는 사람이었다. 그 사실이 위에 내려앉는다.",
      "shape": "box",
      "level": 3,
      "order": 14,
      "ending": false
    },
    {
      "id": "wolf_help",
      "label": "L3 #15 wolf_help\\n화살을 뽑는 순간 늑대가 낮게 으르렁댄다.",
      "title": "화살을 뽑는 순간 늑대가 낮게 으르렁댄다.\n그러나 물지 않는다.\n화살이 빠지자 눈의 붉은빛이 조금 옅어진다.\n늑대는 당신을 한참 바라보다 — 숲 깊은 곳으로 걷기 시작한다.\n따라오라는 것처럼.",
      "shape": "box",
      "level": 3,
      "order": 15,
      "ending": false
    },
    {
      "id": "wolf_calm",
      "label": "L3 #16 wolf_calm\\n유물의 빛이 늑대의 붉은 눈을 감싼다.",
      "title": "유물의 빛이 늑대의 붉은 눈을 감싼다.\n녀석은 천천히 고개를 숙이더니 길을 열어준다.\n유물이 손 안에서 뜨거워진다. 두 번째가 될수록 더 뜨거울 것이다.",
      "shape": "box",
      "level": 3,
      "order": 16,
      "ending": false
    },
    {
      "id": "wizard_confront",
      "label": "L3 #17 wizard_confront\\n마법사는 당신의 눈을 피하지 않는다.",
      "title": "마법사는 당신의 눈을 피하지 않는다.\n긴 침묵.\n'...맞다. 내가 계약서를 번역했다. 내가 조건을 바꿨다.\n왕은 몰랐다.'\n그는 잠시 눈을 감는다.\n'그러나 막을 수 있는 것도 나뿐이다. 나는 계약의 구조를 안다.\n나를 이용할지, 처단할지 — 선택해라.'",
      "shape": "box",
      "level": 3,
      "order": 17,
      "ending": false
    },
    {
      "id": "wizard_listen",
      "label": "L3 #18 wizard_listen\\n마법사의 이야기는 매끄럽다.",
      "title": "마법사의 이야기는 매끄럽다.\n왕이 욕심을 부렸고, 자신은 말렸다고.\n그는 지도를 꺼낸다 — 성 안으로 들어가는 비밀 통로.\n'함께 가자. 내가 증언하겠다.'\n그의 눈이 — 당신의 품 안 유물 쪽으로 한 번, 빠르게 움직인다.",
      "shape": "box",
      "level": 3,
      "order": 18,
      "ending": false
    },
    {
      "id": "wizard_basement",
      "label": "L3 #19 wizard_basement\\n탑 지하.",
      "title": "탑 지하.\n유리 관 여러 개가 늘어서 있다.\n그 안에 — 사람들이 있다. 눈을 뜨고 있지만 움직이지 않는다.\n기억을 잃은 채로 보존된 것들.\n그리고 맨 끝 관에 — 당신이 아는 얼굴이 있다.\n동생이다.",
      "shape": "box",
      "level": 3,
      "order": 19,
      "ending": false
    },
    {
      "id": "villager_helped",
      "label": "L3 #20 villager_helped\\n유물의 빛이 아이의 이마에 닿자 눈이 떠졌다.",
      "title": "유물의 빛이 아이의 이마에 닿자 눈이 떠졌다.\n아이가 엄마를 불렀다.\n여인이 울며 당신의 손을 잡는다.\n마을 사람들이 하나둘 문을 열고 나온다.\n당신은 지금 이 모습이 — 동생이 살았다면 봤을 모습이라는 걸 안다.",
      "shape": "box",
      "level": 3,
      "order": 20,
      "ending": false
    },
    {
      "id": "villager_truth",
      "label": "L3 #21 villager_truth\\n여인의 얼굴이 굳는다.",
      "title": "여인의 얼굴이 굳는다.\n'...그럼 마법사가 우리 아이를 고른 거예요?'\n잠시 후, 그녀가 입술을 깨물며 말한다:\n'탑 지하로 들어가는 문이 있어요. 마법사가 매일 밤 내려가는 거 봤어요.\n한 번도 뭔가를 들고 내려간 적이 없는데 — 항상 뭔가를 들고 올라왔어요.'",
      "shape": "box",
      "level": 3,
      "order": 21,
      "ending": false
    },
    {
      "id": "villager_info",
      "label": "L3 #22 villager_info\\n여인은 잠시 망설이다 말한다:",
      "title": "여인은 잠시 망설이다 말한다:\n'마법사가 매일 밤 성으로 들어가는 걸 봤어요. 왕과 단둘이.\n그리고 아침마다 마을에서 한 명씩 기억을 잃었어요.'\n그녀가 당신을 올려다본다. '도대체 왜 오신 거예요?'\n당신은 대답하지 못한다.",
      "shape": "box",
      "level": 3,
      "order": 22,
      "ending": false
    },
    {
      "id": "castle_gate",
      "label": "L3 #23 castle_gate\\n성문. 두께가 손바닥 두 배는 되는 강철 문.",
      "title": "성문. 두께가 손바닥 두 배는 되는 강철 문.\n위에서 병사들의 발소리.\n달빛 아래, 성은 살아있는 것처럼 느껴진다.\n안에 동생이 있다. 안에 왕이 있다. 안에 계약서가 있다.",
      "shape": "box",
      "level": 3,
      "order": 23,
      "ending": false
    },
    {
      "id": "villager_old",
      "label": "L3 #24 villager_old\\n노인이 오래 침묵한다.",
      "title": "노인이 오래 침묵한다.\n'...그 해에 사라진 사람 중에 젊은 외지인이 있었소.\n마법사가 데려갔다고 봤다는 사람이 있었지.\n탑 지하에 뭔가 있소. 들어간 사람이 없어서 모를 뿐.'\n그는 더 이상 말하지 않는다.",
      "shape": "box",
      "level": 3,
      "order": 24,
      "ending": false
    },
    {
      "id": "gate_sibling",
      "label": "L3 #25 gate_sibling\\n수문장의 표정이 굳는다.",
      "title": "수문장의 표정이 굳는다.\n'...3년 전? 그 해에 실종된 사람이 한둘이 아니었소.'\n그는 더 말하려다 입을 다문다. 위를 힐끗 본다 — 성루 위 병사들.\n쪽지 하나를 손에 쥐어준다. '숲 동굴. 제단 뒤.'",
      "shape": "box",
      "level": 3,
      "order": 25,
      "ending": false
    },
    {
      "id": "ruins",
      "label": "L4 #26 ruins\\n무너진 신전 터. 중앙 제단 위에 유물 하나.",
      "title": "무너진 신전 터. 중앙 제단 위에 유물 하나.\n'아에르돈의 심장' — 계약을 맺을 때 중재자가 왕에게 건넨 것.\n가까이 다가가자 유물이 박동한다. 그 리듬이 — 심장박동과 똑같다.\n제단 옆 벽에 누군가 긁어놓은 글자: '가져가지 마라. 그것이 원한다면.'",
      "shape": "box",
      "level": 4,
      "order": 26,
      "ending": false
    },
    {
      "id": "wolf_grief",
      "label": "L4 #27 wolf_grief\\n당신은 늑대 옆에 한참 앉아 있었다.",
      "title": "당신은 늑대 옆에 한참 앉아 있었다.\n동생도 이렇게 혼자였을까.\n분노가 목 끝까지 차오른다.\n그러나 분노는 지금 당신의 적이 아니다.",
      "shape": "box",
      "level": 4,
      "order": 27,
      "ending": false
    },
    {
      "id": "wolf_leads",
      "label": "L4 #28 wolf_leads\\n늑대가 이끈 곳은 오래된 돌 제단이었다.",
      "title": "늑대가 이끈 곳은 오래된 돌 제단이었다.\n제단 위에 무언가 놓여 있다 — 왕실 인장이 찍힌 서신.\n'계약의 조건: 매 십 년, 왕국 안에서 기억을 가진 자 하나를 바친다.\n증인: 아에르돈 왕, 계약의 중재자 [이름이 불에 탄 흔적]'\n이름은 알아볼 수 없다. 그러나 서명 아래 작은 글씨: '탑의 마법사가 번역함'",
      "shape": "box",
      "level": 4,
      "order": 28,
      "ending": false
    },
    {
      "id": "wizard_dead",
      "label": "L4 #29 wizard_dead\\n마법사가 쓰러졌다.",
      "title": "마법사가 쓰러졌다.\n그의 마지막 말: '...왕좌 왼쪽 기둥. 계약서가 있다. 그걸 태워라.'\n당신은 그 말을 새겼다.\n그리고 — 지하 관 속 사람들이 어떻게 되는지는 아직 모른다.",
      "shape": "box",
      "level": 4,
      "order": 29,
      "ending": false
    },
    {
      "id": "castle_inside",
      "label": "L4 #30 castle_inside\\n성 내부. 벽마다 왕국의 역사.",
      "title": "성 내부. 벽마다 왕국의 역사.\n첫 번째 왕, 계약, 천 년의 번영.\n그리고 지금 — 저주.\n왕좌의 불빛이 보인다. 그 옆 왼쪽 기둥도.",
      "shape": "box",
      "level": 4,
      "order": 30,
      "ending": false
    },
    {
      "id": "basement_smash",
      "label": "L4 #31 basement_smash\\n관이 깨지는 순간 마법사가 나타난다.",
      "title": "관이 깨지는 순간 마법사가 나타난다.\n'멈춰. 계약이 풀리기 전에 꺼내면 그 자리에서 소멸한다.'\n그의 표정에 처음으로 당혹감이 스쳤다.\n당신의 손에 유리 파편이 박혔다. 동생은 아직 눈을 뜨지 않았다.",
      "shape": "box",
      "level": 4,
      "order": 31,
      "ending": false
    },
    {
      "id": "basement_relic",
      "label": "L4 #32 basement_relic\\n유물의 빛이 관을 감쌌다.",
      "title": "유물의 빛이 관을 감쌌다.\n동생의 눈꺼풀이 떨렸다.\n그러나 유물이 세 번째로 쓰이는 순간 — 당신의 손이 투명해지기 시작했다.\n계약이 당신을 담보로 인식하기 시작한 것이다.",
      "shape": "box",
      "level": 4,
      "order": 32,
      "ending": false
    },
    {
      "id": "death_gate",
      "label": "L8 #33 death_gate\\n성문의 병사들이 당신을 에워쌌다.",
      "title": "성문의 병사들이 당신을 에워쌌다.\n무기도, 통로도, 편도 없었다.\n마지막 순간 — 성벽 너머 탑 지하 창문에서 희미한 빛이 새어나오는 게 보였다.\n동생이 있는 곳.\n그 빛이 꺼지기 전에, 당신이 먼저 쓰러졌다.",
      "shape": "box",
      "level": 8,
      "order": 33,
      "ending": true
    },
    {
      "id": "whisper",
      "label": "L5 #34 whisper\\n유물이 손 안에서 뛰기 시작한다.",
      "title": "유물이 손 안에서 뛰기 시작한다.\n목소리가 스며든다. 달콤하다. 그리고 익숙하다.\n— 동생의 목소리다.\n'누나. 나야. 여기 있어. 이걸 써. 나를 꺼내줘.'\n당신은 안다. 이게 진짜일 리 없다는 것을.\n그러나 손이 떨린다.",
      "shape": "box",
      "level": 5,
      "order": 34,
      "ending": false
    },
    {
      "id": "ruins_destroyed",
      "label": "L5 #35 ruins_destroyed\\n유물이 산산조각 나는 순간, 땅이 진동했다.",
      "title": "유물이 산산조각 나는 순간, 땅이 진동했다.\n파편에서 목소리가 흘러나온다:\n'이제 저주는 왕 혼자 짊어진다. 그리고 점점 빨라질 것이다.'\n시간이 없다. 지금 당장 성으로 가야 한다.",
      "shape": "box",
      "level": 5,
      "order": 35,
      "ending": false
    },
    {
      "id": "ruins_lore",
      "label": "L5 #36 ruins_lore\\n신전 깊숙한 곳의 석판:",
      "title": "신전 깊숙한 곳의 석판:\n'심장은 계약의 담보다. 계약이 살아있는 한 심장도 살아있다.\n심장을 세 번 쓰면 계약이 완성되며, 그 사용자가 계약의 새 담보가 된다.'\n그리고 마지막 줄 — 다른 손으로 긁어 쓴 것:\n'중재자는 이 사실을 알고 있다. 왕은 몰랐다.'",
      "shape": "box",
      "level": 5,
      "order": 36,
      "ending": false
    },
    {
      "id": "throne",
      "label": "L5 #37 throne\\n왕은 왕좌에 앉아 있다. 그 눈빛은 공허하다.",
      "title": "왕은 왕좌에 앉아 있다. 그 눈빛은 공허하다.\n저주가 몸을 잠식해가고 있다는 걸 한눈에 알 수 있다.\n그가 천천히 입을 연다:\n'오래 기다렸다.'\n그리고 — 당신의 얼굴을 보며, 처음으로 표정이 흔들린다.\n'...너. 3년 전 그 아이의.'\n그가 알고 있었다.",
      "shape": "box",
      "level": 5,
      "order": 37,
      "ending": false
    },
    {
      "id": "castle_contract",
      "label": "L5 #38 castle_contract\\n기둥 뒤, 석판 안에 양피지 하나.",
      "title": "기둥 뒤, 석판 안에 양피지 하나.\n계약서.\n조건들이 쓰여 있다. 그리고 번역이 얼마나 교묘하게 바뀌었는지.\n왕의 서명 옆에 — 이 계약서를 태우면 계약이 소멸한다는 조항이 있다.\n마법사가 자신의 탈출구를 만들어둔 것인지, 아니면 진짜 후회였는지.",
      "shape": "box",
      "level": 5,
      "order": 38,
      "ending": false
    },
    {
      "id": "castle_lore",
      "label": "L5 #39 castle_lore\\n성벽 기록의 마지막 줄:",
      "title": "성벽 기록의 마지막 줄:\n'저주는 계약의 문구 안에 있다. 왕이 스스로 계약을 부정하면 저주는 풀린다.\n그러나 왕이 계약을 부정하려면 — 왕은 모든 것을 내려놓아야 한다.\n왕좌도. 이름도. 목숨도.'\n당신은 잠시 멈춘다.\n왕이 그럴 수 있는 사람인지 — 모른다.",
      "shape": "box",
      "level": 5,
      "order": 39,
      "ending": false
    },
    {
      "id": "doom_ending",
      "label": "L8 #40 doom_ending\\n유물이 세 번째로 빛을 냈다.",
      "title": "유물이 세 번째로 빛을 냈다.\n계약이 완성됐다.\n당신의 몸이 빛 속으로 흡수되기 시작했다.\n\n마지막 순간, 동생의 관이 열렸다.\n동생이 눈을 떴다. 당신을 보았다.\n그러나 당신은 이미 형체가 없었다.\n\n동생이 부르는 이름이 들렸다.\n그리고 아무것도 들리지 않았다.\n\n아에르돈의 저주는 풀렸다.\n그리고 새로운 담보가 생겼다.",
      "shape": "box",
      "level": 8,
      "order": 40,
      "ending": true
    },
    {
      "id": "whisper_refused",
      "label": "L6 #41 whisper_refused\\n유물이 잠시 침묵한다.",
      "title": "유물이 잠시 침묵한다.\n그러다 목소리가 바뀐다 — 이번엔 차갑다.\n'현명하군. 그럼 그냥 가져만 다녀. 쓰지 않으면 아무 일도 없으니까.'\n당신은 유물을 품에 넣었다. 그 말이 어째서 더 무섭다.",
      "shape": "box",
      "level": 6,
      "order": 41,
      "ending": false
    },
    {
      "id": "whisper_deal",
      "label": "L6 #42 whisper_deal\\n유물이 답한다.",
      "title": "유물이 답한다.\n'네 동생은 계약의 담보가 됐다. 이 유물 안에 있지.\n나를 세 번 쓰면 계약이 완성되고 — 그 때 네가 담보가 된다. 동생은 나온다.'\n침묵.\n이것이 사실인지, 거짓인지, 알 방법이 없다.",
      "shape": "box",
      "level": 6,
      "order": 42,
      "ending": false
    },
    {
      "id": "true_ending",
      "label": "L8 #43 true_ending\\n검이 저주의 뿌리를 끊었다.",
      "title": "검이 저주의 뿌리를 끊었다.\n유물이 그 상처를 봉했다.\n마법사가 직접 계약의 마지막 줄을 지웠다.\n그리고 — 왕이 스스로 왕좌에서 내려섰다.\n\n지하의 관들이 열렸다. 사람들이 눈을 떴다.\n동생이 당신의 이름을 불렀다.\n\n마법사는 사라졌다. 찾는 사람도 없었다.\n왕은 왕국의 첫 번째 시민이 됐다.\n당신과 동생은 국경 너머로 걸어나갔다.\n\n아에르돈은 다시 새를 노래하게 했다.\n그것으로 충분했다.",
      "shape": "box",
      "level": 8,
      "order": 43,
      "ending": true
    },
    {
      "id": "king_ending",
      "label": "L8 #44 king_ending\\n왕의 피가 왕좌를 적셨다.",
      "title": "왕의 피가 왕좌를 적셨다.\n저주는 풀렸다.\n그러나 — 지하의 관들은 그대로였다.\n계약이 왕의 죽음으로 풀리지 않았다.\n\n당신은 텅 빈 왕좌 앞에 홀로 서 있었다.\n동생은 아직 지하에 있다.\n새 왕이라는 소문이 퍼졌지만, 당신은 왕좌에 앉지 않았다.\n\n매일 밤 지하로 내려갔다. 관 앞에 앉아 이름을 불렀다.\n계약을 푸는 법을 찾는 데 남은 생을 바쳤다.\n끝내 찾았는지는 — 아무도 모른다.",
      "shape": "box",
      "level": 8,
      "order": 44,
      "ending": true
    },
    {
      "id": "hero_ending",
      "label": "L8 #45 hero_ending\\n왕이 계약서를 읽었다.",
      "title": "왕이 계약서를 읽었다.\n긴 침묵.\n'...나는 몰랐다. 진짜로.'\n눈물이 아니었다. 그보다 더 깊은 무언가였다.\n\n왕은 그 자리에서 계약을 소리 내어 부정했다.\n왕좌를 포기하고, 이름을 내려놓고, 저주를 스스로 끊었다.\n그것은 그가 살면서 처음으로 한 — 진짜 선택이었다.\n\n지하의 관들이 열렸다. 동생이 눈을 떴다.\n왕국은 천천히 숨을 되찾기 시작했다.\n당신은 동생의 손을 잡고 성문을 걸어나왔다.\n아무도 당신의 이름을 기억하지 않아도, 그것으로 충분했다.",
      "shape": "box",
      "level": 8,
      "order": 45,
      "ending": true
    },
    {
      "id": "sacrifice_ending",
      "label": "L8 #46 sacrifice_ending\\n당신이 무릎을 꿇는 순간, 유물이 스스로 빛을 냈다.",
      "title": "당신이 무릎을 꿇는 순간, 유물이 스스로 빛을 냈다.\n왕이 눈을 크게 떴다.\n'당신은... 계약을 짊어지려는 건가?'\n\n당신은 아무 말도 하지 않았다.\n동생을 꺼낼 수 있다면.\n그것만 생각했다.\n\n빛이 당신을 감쌌다.\n저주가 사라졌다. 관들이 열렸다. 동생이 눈을 떴다.\n\n그러나 당신은 아에르돈에서 영영 떠나지 못하게 됐다.\n계약의 새 담보. 형체도, 이름도 없이.\n\n동생이 성문 밖으로 걸어나가는 걸 당신은 보았다.\n동생은 한 번도 뒤를 돌아보지 않았다.\n당신이 거기 있는 걸 — 몰랐으니까.",
      "shape": "box",
      "level": 8,
      "order": 46,
      "ending": true
    },
    {
      "id": "contract_burned",
      "label": "L6 #47 contract_burned\\n양피지가 타오르는 순간 성 전체가 흔들렸다.",
      "title": "양피지가 타오르는 순간 성 전체가 흔들렸다.\n저주가 풀리기 시작한다.\n그러나 — 지하의 관들은?\n동생은?",
      "shape": "box",
      "level": 6,
      "order": 47,
      "ending": false
    },
    {
      "id": "sibling_free",
      "label": "L7 #48 sibling_free\\n계약이 소멸하자 관들이 저절로 열렸다.",
      "title": "계약이 소멸하자 관들이 저절로 열렸다.\n사람들이 하나씩 눈을 떴다.\n당신은 동생 앞에 무릎을 꿇었다.\n동생이 눈을 뜨고 — 당신의 이름을 불렀다.\n그 한 마디에, 3년이 녹았다.",
      "shape": "box",
      "level": 7,
      "order": 48,
      "ending": false
    }
  ],
  "edges": [
    {
      "from": "start",
      "to": "forest"
    },
    {
      "from": "start",
      "to": "village"
    },
    {
      "from": "start",
      "to": "castle_gate_early"
    },
    {
      "from": "forest",
      "to": "cave"
    },
    {
      "from": "forest",
      "to": "deep_forest"
    },
    {
      "from": "forest",
      "to": "village"
    },
    {
      "from": "village",
      "to": "wizard"
    },
    {
      "from": "village",
      "to": "villager"
    },
    {
      "from": "village",
      "to": "village_wall"
    },
    {
      "from": "castle_gate_early",
      "to": "village"
    },
    {
      "from": "castle_gate_early",
      "to": "gate_talk"
    },
    {
      "from": "castle_gate_early",
      "to": "forest"
    },
    {
      "from": "cave",
      "to": "cave_sword_taken"
    },
    {
      "from": "cave",
      "to": "secret_path"
    },
    {
      "from": "cave",
      "to": "cave_inspect"
    },
    {
      "from": "deep_forest",
      "to": "wolf_win"
    },
    {
      "from": "deep_forest",
      "to": "wolf_help"
    },
    {
      "from": "deep_forest",
      "to": "wolf_calm"
    },
    {
      "from": "wizard",
      "to": "wizard_confront"
    },
    {
      "from": "wizard",
      "to": "wizard_listen"
    },
    {
      "from": "wizard",
      "to": "wizard_basement"
    },
    {
      "from": "villager",
      "to": "villager_helped"
    },
    {
      "from": "villager",
      "to": "villager_truth"
    },
    {
      "from": "villager",
      "to": "villager_info"
    },
    {
      "from": "village_wall",
      "to": "wizard"
    },
    {
      "from": "village_wall",
      "to": "castle_gate"
    },
    {
      "from": "village_wall",
      "to": "villager_old"
    },
    {
      "from": "gate_talk",
      "to": "wizard"
    },
    {
      "from": "gate_talk",
      "to": "village"
    },
    {
      "from": "gate_talk",
      "to": "gate_sibling"
    },
    {
      "from": "cave_sword_taken",
      "to": "secret_path"
    },
    {
      "from": "cave_sword_taken",
      "to": "forest"
    },
    {
      "from": "cave_sword_taken",
      "to": "cave_inspect"
    },
    {
      "from": "secret_path",
      "to": "ruins"
    },
    {
      "from": "secret_path",
      "to": "village"
    },
    {
      "from": "secret_path",
      "to": "ruins"
    },
    {
      "from": "cave_inspect",
      "to": "secret_path"
    },
    {
      "from": "cave_inspect",
      "to": "secret_path"
    },
    {
      "from": "cave_inspect",
      "to": "village"
    },
    {
      "from": "wolf_win",
      "to": "ruins"
    },
    {
      "from": "wolf_win",
      "to": "village"
    },
    {
      "from": "wolf_win",
      "to": "wolf_grief"
    },
    {
      "from": "wolf_help",
      "to": "wolf_leads"
    },
    {
      "from": "wolf_help",
      "to": "ruins"
    },
    {
      "from": "wolf_help",
      "to": "village"
    },
    {
      "from": "wolf_calm",
      "to": "ruins"
    },
    {
      "from": "wolf_calm",
      "to": "wolf_leads"
    },
    {
      "from": "wolf_calm",
      "to": "village"
    },
    {
      "from": "wizard_confront",
      "to": "castle_gate"
    },
    {
      "from": "wizard_confront",
      "to": "wizard_dead"
    },
    {
      "from": "wizard_confront",
      "to": "castle_inside"
    },
    {
      "from": "wizard_listen",
      "to": "castle_inside"
    },
    {
      "from": "wizard_listen",
      "to": "castle_gate"
    },
    {
      "from": "wizard_listen",
      "to": "wizard_confront"
    },
    {
      "from": "wizard_basement",
      "to": "basement_smash"
    },
    {
      "from": "wizard_basement",
      "to": "wizard_confront"
    },
    {
      "from": "wizard_basement",
      "to": "basement_relic"
    },
    {
      "from": "villager_helped",
      "to": "wizard"
    },
    {
      "from": "villager_helped",
      "to": "village_wall"
    },
    {
      "from": "villager_helped",
      "to": "castle_gate"
    },
    {
      "from": "villager_truth",
      "to": "wizard"
    },
    {
      "from": "villager_truth",
      "to": "castle_gate"
    },
    {
      "from": "villager_truth",
      "to": "wizard"
    },
    {
      "from": "villager_info",
      "to": "villager_truth"
    },
    {
      "from": "villager_info",
      "to": "village"
    },
    {
      "from": "villager_info",
      "to": "wizard"
    },
    {
      "from": "castle_gate",
      "to": "castle_inside"
    },
    {
      "from": "castle_gate",
      "to": "death_gate"
    },
    {
      "from": "castle_gate",
      "to": "castle_inside"
    },
    {
      "from": "villager_old",
      "to": "wizard"
    },
    {
      "from": "villager_old",
      "to": "villager"
    },
    {
      "from": "villager_old",
      "to": "castle_gate"
    },
    {
      "from": "gate_sibling",
      "to": "forest"
    },
    {
      "from": "gate_sibling",
      "to": "village"
    },
    {
      "from": "ruins",
      "to": "whisper"
    },
    {
      "from": "ruins",
      "to": "ruins_destroyed"
    },
    {
      "from": "ruins",
      "to": "ruins_lore"
    },
    {
      "from": "wolf_grief",
      "to": "ruins"
    },
    {
      "from": "wolf_grief",
      "to": "village"
    },
    {
      "from": "wolf_leads",
      "to": "ruins"
    },
    {
      "from": "wolf_leads",
      "to": "wizard"
    },
    {
      "from": "wolf_leads",
      "to": "village"
    },
    {
      "from": "wizard_dead",
      "to": "castle_gate"
    },
    {
      "from": "wizard_dead",
      "to": "wizard_basement"
    },
    {
      "from": "castle_inside",
      "to": "throne"
    },
    {
      "from": "castle_inside",
      "to": "castle_contract"
    },
    {
      "from": "castle_inside",
      "to": "castle_lore"
    },
    {
      "from": "basement_smash",
      "to": "wizard_confront"
    },
    {
      "from": "basement_smash",
      "to": "castle_gate"
    },
    {
      "from": "basement_smash",
      "to": "basement_relic"
    },
    {
      "from": "basement_relic",
      "to": "doom_ending"
    },
    {
      "from": "basement_relic",
      "to": "wizard_confront"
    },
    {
      "from": "whisper",
      "to": "forest"
    },
    {
      "from": "whisper",
      "to": "whisper_refused"
    },
    {
      "from": "whisper",
      "to": "whisper_deal"
    },
    {
      "from": "ruins_destroyed",
      "to": "castle_gate"
    },
    {
      "from": "ruins_destroyed",
      "to": "village"
    },
    {
      "from": "ruins_destroyed",
      "to": "forest"
    },
    {
      "from": "ruins_lore",
      "to": "whisper"
    },
    {
      "from": "ruins_lore",
      "to": "ruins_destroyed"
    },
    {
      "from": "ruins_lore",
      "to": "wizard"
    },
    {
      "from": "throne",
      "to": "true_ending"
    },
    {
      "from": "throne",
      "to": "king_ending"
    },
    {
      "from": "throne",
      "to": "doom_ending"
    },
    {
      "from": "throne",
      "to": "hero_ending"
    },
    {
      "from": "throne",
      "to": "sacrifice_ending"
    },
    {
      "from": "castle_contract",
      "to": "contract_burned"
    },
    {
      "from": "castle_contract",
      "to": "throne"
    },
    {
      "from": "castle_contract",
      "to": "throne"
    },
    {
      "from": "castle_lore",
      "to": "throne"
    },
    {
      "from": "whisper_refused",
      "to": "castle_gate"
    },
    {
      "from": "whisper_refused",
      "to": "village"
    },
    {
      "from": "whisper_refused",
      "to": "ruins_lore"
    },
    {
      "from": "whisper_deal",
      "to": "village"
    },
    {
      "from": "whisper_deal",
      "to": "village"
    },
    {
      "from": "whisper_deal",
      "to": "wizard"
    },
    {
      "from": "contract_burned",
      "to": "throne"
    },
    {
      "from": "contract_burned",
      "to": "sibling_free"
    }
  ]
};
